...
var viewer = {
    "id": "bd20mm",
    "name": "Bay Delta 20-mm Zooplankton Distribution Map",
    "title": "CDFW Bay Delta Region 20-mm Zooplankton Distribution Survey",
    "subtitle": "Organism Per Cupic Meter",
    "homepage": "https://www.wildlife.ca.gov/Conservation/Delta/20mm-Survey",
    "mappage": "http://www.dfg.ca.gov/delta/data/20mm/CPUE_zoomap.asp",
    "docTitle": "Zooplankton Map",
    "url": "delta/zoomap.html",
...
var app = {
    ...
    survey: 9,
    species: "101",
    commonname: "All Calanoid Copepods",
    layers: {
        "bd20mmStations": {
            url: "//services2.arcgis.com/Uq9r85Potqm3MfRV/ArcGIS/rest/services/bd20mmgdb/FeatureServer/0"
        },
        "bd20mmZooplanktonDistribution": {
            url: "//services2.arcgis.com/Uq9r85Potqm3MfRV/ArcGIS/rest/services/bd20mmgdb/FeatureServer/2"
        },
        "bd20mmZooplanktonAreaPercentage": {
            url: "//services2.arcgis.com/Uq9r85Potqm3MfRV/ArcGIS/rest/services/bd20mmgdb/FeatureServer/5"
        },
        "bdSpecies": {
...
var defaultColor = "dodgerblue";//#1e90ff "deepskyblue#00bfff";
var geomSer = null;
...
require([
    ...
    //--------------------- LOAD MAP DATA ---------------------
    var dataUrl = app.layers["bd20mmZooplanktonDistribution"].url;
    var stationsUrl = app.layers["bd20mmStations"].url;
    var tableUrl = app.layers["bd20mmZooplanktonAreaPercentage"].url;
    ...
    // Removes all the widgets from the top left of the view--20170628
    //REF--https://developers.arcgis.com/javascript/latest/api-reference/esri-views-MapView.html#ui
    ...
    var classbreaks = [
        ...
        { min: 0, max: 10, size: 6, color: "#1e90ff" },
        { min: 10, max: 50, size: 12, color: "#1e90ff" },
        { min: 50, max: 100, size: 18, color: "#1e90ff" },
        { min: 100, max: 200, size: 24, color: "#1e90ff" },
        { min: 200, max: 500, size: 36, color: "#1e90ff" }
    ];
    ...
    var def = "CommonName = '" + app.commonname + "' AND Year = " + app.year + " AND Survey = " + app.survey;
    //def = "Species = '" + app.species + "'";
    var bd20mmZooplanktonDistribution = new FeatureLayer({
        url: dataUrl,
        renderer: renderer,
        definitionExpression: def
    });
    //bd20mmZooplanktonDistribution.setDefinitionExpression(def);
    map.add(bd20mmZooplanktonDistribution);
    ...
    var legend = new Legend({
        ...
        layerInfos: [
        {
            ...
            layer: bd20mmZooplanktonDistribution,
            title: "CPUE"
        }]
    }, "map-legend");
    ...
    function rerender(maxval) {
        ...
        var classbreaks = [
        { min: 0, max: 0, size: 5, color: "white" },
        { min: 0, max: incr, size: 6, color: "#1e90ff" },
        { min: incr, max: (incr * 2), size: 12, color: "#1e90ff" },
        { min: (incr * 2), max: (incr * 3), size: 18, color: "#1e90ff" },
        { min: (incr * 3), max: (incr * 4), size: 24, color: "#1e90ff" },
        { min: (incr * 4), max: (incr * 5), size: 36, color: "#1e90ff" }
        ];
        for (var i = 0; i < classbreaks.length; i++) {
            renderer.addClassBreakInfo({
                minValue: classbreaks[i].min,
    ...
    function drawMap() {
        ...
        def = "CommonName = '" + commonName + "' AND Survey = " + survey + " AND Year = " + year;
        def = "Species = '" + species + "' AND Survey = " + survey + " AND Year = " + year;
        bd20mmZooplanktonDistribution.definitionExpression = def;
    ...
    function queryData(def) {
        ...
        queryTask.execute(query).then(function (results) {
            ...
            var cols = ["Station", "Temp", "TopEC", "CommonName", "CPUE"];
            ...
            for (var i = 0; i < features.length; i++) {
                ...
                bd20mmZooplanktonDistribution.renderer = renderer;
            }
            queryPercentages();
...
/*
 *-------------------------------- NOTES -------------------------------------------------
20170613 Created zooplankton map app based on bd20mmmap and using bd20mmgdb service.
20170615--line:669 Changed column names "SurfaceTemp", "SurfaceEC", "NumOfTows" to Temp, TopEC, or Species.
