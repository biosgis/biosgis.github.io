...
function init() {
    ...
    pickonload('Survey');
    listnocatch();
    // stationlabels.on('createfeature', function (e) {
    //     // var id = e.feature.id;
    //     // var feature = stationlabels.getFeature(id);
...
function drawMap() {
    ...
    var sql = "Year = " + $("Year").value + " AND Survey = " + $("Survey").value + " AND (Species = '" + $("Species").value + "' OR Species = 'SPECIES')";
...
function hidepct() {
    ...
    // dohide('baylabel');
    // dohide('deltalabel');
    // dohide('upstreamlabel');//=FAIL
    labelpcts();
}
function listspecies() {
    var query = L.esri.query({
        url: app.layers.bd20mmzoo.url
    });
...
function listyears() {
    ...
    var sql = "Year IS NOT NULL";
    query.where(sql).fields(["Year"])
        .orderBy('Year', 'DESC').distinct();
    query.run(function (error, featureCollection, response) {
        addmsg('CALLBACK listyears queryrun: ' + featureCollection.features.length + ' features found');
        var features = featureCollection.features;
        var ar = [];
        features.forEach(fi => {
            ar.push([fi.properties['Year'], fi.properties['Year']]);
        });
...
function listsurveys() {
    ...
    let sqlwhere = "Species = " + $('Species').value + " AND Year = " + $('Year').value;
    let sqlorderby = "Survey ASC";
    let distinct = true;
    let queryurl = app.layers.bd20mmzoopct.url;
}
function querypct() {
    let sql = "Species = " + $('Species').value + " AND Year = " + $('Year').value + " AND Survey = " + $("Survey").value;
    addmsg('DO querypct ' + sql);
    var query = L.esri.query({
        url: app.layers.bd20mmzoopct.url
    });
    ...
    query.run(function (error, featureCollection, response) {
        ...
        // for (key in pcts) {
        //     var val = pcts[key];
        //     // $(key.toLowerCase() + 'label').innerHTML = val + '%';
        //     // unhide(key.toLowerCase() + 'label');//=FAIL
        // }
    });
}
...
cpuefields = ['Station', 'Temp', 'TopEC', 'CPUE'];//, 'SurveyStartDate', 'SurveyEndDate'
// 'Year', 'Survey', 'Species', 'CommonName', 
...
function drawstations(features) {
    ...
    stationsmarked = L.esri.featureLayer({
        ...
        pointToLayer: function (geojson, latlng) {
            ...
            let val = stvals[stid];
            msgput(stid, val);
            // *** CPUE FIELD NOT IN STATIONS LAYER SO IT IS UNDFF
